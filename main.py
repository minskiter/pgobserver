...
file_handler = logging.FileHandler("{}.log".format(datetime.now().strftime("%y-%m-%d")))
...
def get_program_status(pid):
    if psutil.pid_exists(pid):
        process = psutil.Process(pid)
        logger.info("获取了进程[{}] {} 的状态{},已运行{}s".format(pid,process.name(),process.status(),int(datetime.now().timestamp()-process.create_time())," ".join(process.cmdline())))
        return process.name(),process.status(),process.cpu_times(),process.create_time(),process.cmdline()
    logger.info("进程[{}]当前不存在".format(pid))
    return None

def send_email_notify(username, password, emails, subject, message):
    from_addr = username
    to_addr = emails
...
def background_service(s=2,stop_event=None):
    try:
...
if __name__=='__main__':
    ...
    if not args.d:
        ...
        try:
            daemon = multiprocessing.Process(target=background_service,args=(args.interval,event))
            daemon.daemon = True
            daemon.start()
            daemon.join()
        except KeyboardInterrupt:
